  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', width: NODE_W, background: '#161b22', border: '1px solid #2a313a', borderRadius: 10, overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.25)', color: '#fff' }}>
      ...
      <div style={{ padding: '7px 10px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, borderBottom: '1px solid #2a313a', flexWrap: 'wrap' }}>
        ...
        <Button className="nodrag" size="mini" icon={<IconMindMapping />} disabled={!hasText || busy} onClick={() => onFloorPlan && onFloorPlan(id)} title="Schematic — render this scene's overhead blocking map from the brief, verbatim (parties, moves, the AXIS of action). Edit it like any image; attach it to a SHOT card to project camera-relative blocking into the prompt.">Schematic</Button>
